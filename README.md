...
* **Прямая адресация $A0 \to A1$:** На схемах линия $A0$ подключается к линии $A1$ слота.
...
* **Логика переключения банков:**
  ...
  * **Управление адресацией ($A{18}\dots A{20}$):** 
    * Управляется линиями $A{18}\dots A{20}$ и записи в `REG(inst8)` , `REG(inst9)` выставляет нужный адрес на линии Flash-памяти $'ROM_A'{18}\dots 'ROM_A'{23}$.
...
* **Элементная база:** **7 регистров**, **6 мультиплексоров** и поддержка памяти **FRAM / SRAM (32 КБ)**.
  ...
  * **Управление адресацией ($A{18}\dots A{20}$):** 
    * Управляется линиями $A{18}\dots A{20}$ и записи регистров $REG{1}\dots REG{7}$ выставляет нужный адрес на линии Flash-памяти $'ROM_A'{18}\dots 'ROM_A'{23}$.
